...
typedef struct {
    ...
    struct event_base *base;    /* libevent handle this thread uses */ //线程使用的event_base
    struct event notify_event;  /* listen event for notify pipe */ //用于监听管道读事件的event
    int notify_receive_fd;      /* receiving end of notify pipe */ //管道读端的fd
    int notify_send_fd;         /* sending end of notify pipe */ //管道写端的fd
